...
canDataset1 = read_data(r"C:\Users\rh517\Desktop\Current\10 M13 FE Complain\rear.csv")
  ...
  for df, col, label in zip([sltCAN1, sltCAN2], [col1, col2], ['A','B']):
    with col:
      ...
      with subcol4:
        ...
        st.metric(label='行驶车速(km/h):', value=np.round(df.loc[df['Vehicle_Speed']>2, 'Vehicle_Speed'].mean(),2))
      # with subcol5:
...
with tab2:
  ...
  for col, df in zip([col21, col22], [sltCAN1, sltCAN2]):
    with col:
      pie_CCPO = fun.plotPie('热管理', df, 'Combustion_Control_Path_Owner')
